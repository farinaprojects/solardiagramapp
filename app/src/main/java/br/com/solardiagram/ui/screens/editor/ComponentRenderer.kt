...
object ComponentRenderer {
    ...
    private fun drawAcBus(
        ...
        val overall = acBusOverallRect(component)
        val style = styleColors(isSelected, isDragging, isError, isWarning)
    ...
    private fun componentWorldBounds(component: Component): Rect {
        val c = component.transform.position
        ...
        val base = when (component.type) {
            ...
            ComponentType.BARL,
            ComponentType.BARN,
            ComponentType.BARPE -> Rect(c.x - BAR_W / 2f - PORT_R, c.y - BAR_H / 2f - 10f - PORT_R, c.x + BAR_W / 2f + PORT_R, c.y + BAR_H / 2f + 10f + PORT_R)

            ComponentType.QDG -> Rect(c.x - QDG_W / 2f - PORT_R, c.y - QDG_H / 2f - PORT_R, c.x + QDG_W / 2f + PORT_R, c.y + QDG_H / 2f + PORT_R)
            ComponentType.BREAKER -> {
                val breakerW = breakerWidth(component)
                Rect(c.x - breakerW / 2f - PORT_R, c.y - BREAKER_H / 2f - PORT_R, c.x + breakerW / 2f + PORT_R, c.y + BREAKER_H / 2f + PORT_R)
            }
            ComponentType.DPS -> Rect(c.x - DPS_W / 2f - PORT_R, c.y - DPS_H / 2f - PORT_R, c.x + DPS_W / 2f + PORT_R, c.y + DPS_H / 2f + PORT_R)
            ComponentType.GROUND_BAR -> Rect(c.x - GROUND_W / 2f - PORT_R, c.y - GROUND_H / 2f - PORT_R, c.x + GROUND_W / 2f + PORT_R, c.y + GROUND_H / 2f + PORT_R)
            ComponentType.LOAD -> {
                val (loadW, loadH) = loadDimensions(component)
                Rect(c.x - loadW / 2f - PORT_R, c.y - loadH / 2f - PORT_R, c.x + loadW / 2f + PORT_R, c.y + loadH / 2f + PORT_R)
            }
        }
    ...
    private fun simpleBarPortWorldPosition(component: Component, port: Port): Point2 {
        ...
        val isOut = id.contains("_out") || id == "out" || port.side == br.com.solardiagram.domain.model.PortSide.BOTTOM
    ...
    private fun acBusPortWorldPosition(component: Component, port: Port): Point2 {
        ...
        val id = port.spec?.id ?: port.name.lowercase()
        val isOut = id.endsWith("_out") || port.name.uppercase().startsWith("OUT ") || port.side == br.com.solardiagram.domain.model.PortSide.BOTTOM
    ...
    private fun breakerPortWorldPosition(component: Component, port: Port): Point2 {
        ...
        val yOffsets = when (poles) {
            br.com.solardiagram.domain.model.BreakerPole.P1 -> listOf(body.center.y)
            br.com.solardiagram.domain.model.BreakerPole.P2 -> listOf(body.top + 10.5f, body.bottom - 10.5f)
            br.com.solardiagram.domain.model.BreakerPole.P3 -> listOf(body.top + 9f, body.center.y, body.bottom - 9f)
            br.com.solardiagram.domain.model.BreakerPole.P4 -> listOf(body.top + 7.5f, body.top + 15.5f, body.bottom - 15.5f, body.bottom - 7.5f)
        }
        fun phaseIndex(phase: br.com.solardiagram.domain.model.ElectricalPhase): Int = when (phase) {
            br.com.solardiagram.domain.model.ElectricalPhase.L1 -> 0
            br.com.solardiagram.domain.model.ElectricalPhase.L2 -> 1
            br.com.solardiagram.domain.model.ElectricalPhase.L3 -> 2
            br.com.solardiagram.domain.model.ElectricalPhase.N -> when (poles) {
                br.com.solardiagram.domain.model.BreakerPole.P2 -> 1
                br.com.solardiagram.domain.model.BreakerPole.P4 -> 3
                else -> 0
            }
            else -> 0
        }
        val phase = port.spec?.phase ?: br.com.solardiagram.domain.model.ElectricalPhase.L1
        val y = yOffsets[phaseIndex(phase).coerceIn(0, yOffsets.lastIndex)]
        val isLoad = port.spec?.terminalRole == br.com.solardiagram.domain.model.PhysicalTerminalRole.LOAD || (port.spec?.id ?: "").startsWith("load_")
    ...
    private fun loadPortWorldPosition(component: Component, port: Port): Point2 {
        ...
        val phase = port.spec?.phase
        val yMap = when (loadPhase(component)) {
            br.com.solardiagram.domain.model.SystemPhase.MONO -> mapOf(
                br.com.solardiagram.domain.model.ElectricalPhase.L1 to body.center.y - 9f,
                br.com.solardiagram.domain.model.ElectricalPhase.N to body.center.y + 9f
            )
            br.com.solardiagram.domain.model.SystemPhase.BI -> mapOf(
                br.com.solardiagram.domain.model.ElectricalPhase.L1 to body.center.y - 18f,
                br.com.solardiagram.domain.model.ElectricalPhase.N to body.center.y,
                br.com.solardiagram.domain.model.ElectricalPhase.L2 to body.center.y + 18f
            )
            br.com.solardiagram.domain.model.SystemPhase.TRI -> mapOf(
                br.com.solardiagram.domain.model.ElectricalPhase.L1 to body.center.y - 26f,
                br.com.solardiagram.domain.model.ElectricalPhase.L2 to body.center.y - 9f,
                br.com.solardiagram.domain.model.ElectricalPhase.L3 to body.center.y + 9f,
                br.com.solardiagram.domain.model.ElectricalPhase.N to body.center.y + 26f
            )
        }

        return when (phase) {
            br.com.solardiagram.domain.model.ElectricalPhase.PE -> Point2(c.x, body.bottom + PORT_R)
